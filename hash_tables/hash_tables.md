...
Для решения этого мы можем реализовать метку `deleted`, означающую, что элемент в этой ячейке был удален.
...
Поиск происходит так - вычисляем хеш, переходим в ячейку по этому индекс, сравниваем элементы - 
если они не равны, идем вниз, пока не найдем исходный или пока не найдем свободную ячейку
...
Займет это все `O(1)` в зависимости от хеш-функции

### Реализация

Rust - [open_address_hashing.rs](open_address_hashing.rs)

Удобно делать хеш-таблицу с открытой адресацией в Rust; создадим `enum`, показывающий состояние ячейки:
